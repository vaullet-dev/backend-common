 ...
 * called — it exists only so that the <em>real</em> chain can be assembled. That matters: the tests
 * then exercise the same filter chain, the same authority mapping and the same deny-by-default rules
 * that run in production, rather than a permissive test-only chain that would make the security
 * tests meaningless.
 *
