...
/**
 * Builds the one error body a Vaullet service returns — RFC 9457 {@code problem+json} in the shape
 * ADR-011, §7 fixes.
 *
 * <p>A separate type rather than a private method on {@link ApiExceptionHandler} because a service
 * that needs a second {@code @RestControllerAdvice} — bridging a legacy exception hierarchy, say —
 * must be able to produce an <em>identical</em> body from it. Two advices producing
 * <em>almost</em> the same body is how a client ends up branching on a {@code code} that is present
 * on some errors and absent on others. One function, one shape, every caller.
 *
